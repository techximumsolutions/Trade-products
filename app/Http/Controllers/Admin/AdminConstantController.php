...
class AdminConstantController extends Controller
{
	...
	// for popup gallery
	public function constantGallery(Request $request){
		if(session('application_setting_view')==0){
			print Lang::get("labels.You do not have to access this route");
		}else{
		...
		$gallery_images = DB::table('constant_gallery')->orderBy('constant_gallery.gallery_id','ASC')->paginate(20);
		
		$result['message'] = $message;
		$result['galleries'] = $gallery_images;
		
		return view("admin.constantgallery", $title)->with('result', $result);
		}
	}
	//Add Gallery Images
	public function addconstantGallery(Request $request){
		$title = array('pageTitle' => Lang::get("labels.AddConstantGallery"));
		...
		return view("admin.addconstantgallery", $title)->with('result', $result);
	}
	//Add Gallery to DB	
	public function addNewconstantGallery(Request $request){
		if(session('application_setting_update')==0){
			print Lang::get("labels.You do not have to access this route");
		}else{
		...
		// }else{
			...
			if($request->hasFile('newImage') and in_array($request->newImage->extension(), $extensions)){
				$image = $request->newImage;
				$fileName = time().'.'.$image->getClientOriginalName();
				$image->move('resources/assets/images/constant_gallery/', $fileName);
				$uploadImage = 'resources/assets/images/constant_gallery/'.$fileName; 
			}else{
				$uploadImage = '';
			}
			
			
			DB::table('constant_gallery')->insert([
					'gallery_title'  		 =>   $request->type,
					'date_added'	 		 =>   date('Y-m-d H:i:s'),
					'gallery_image'			 =>	  $uploadImage,
					'gallery_url'	 		 =>   $request->banners_url,
					'status'	 			 =>   $request->status,
					]);
			return redirect()->back()->with('success', Lang::get("labels.GalleryAddedMessage"));
			//}
		}
	}
	//delete Gallery Image
	public function deleteconstantgallery(Request $request){
		if(session('application_setting_update')==0){
			print Lang::get("labels.You do not have to access this route");
		}else{
		DB::table('constant_gallery')->where('gallery_id',$request->galleries_id)->delete();
		return redirect()->back()->withErrors([Lang::get("labels.BannerDeletedMessage")]);
		}
	}
	//edit Gallery
	//editTaxClass
	public function editconstantGallery(Request $request){		
		$title = array('pageTitle' => Lang::get("labels.EditBanner"));
		$result = array();		
		$result['message'] = array();
		
		$gallery = DB::table('constant_gallery')->where('gallery_id', $request->id)->get();
		...
		return view("admin.editconstantgallery",$title)->with('result', $result);
	}
	//Update the gallery
	public function updateconstantgallery(Request $request){
		if(session('application_setting_update')==0){
			print Lang::get("labels.You do not have to access this route");
		}else{
			
			// $exist = DB::table('constant_gallery')->where('gallery_id','!=',$request->id)->get();
				
			// if(count($exist)>0){			
			// 	return redirect()->back()->with('error', Lang::get("labels.constantBannerErrorMessage"));
			// }else{	
			$title = array('pageTitle' => Lang::get("labels.EditGallery"));
			$type = $request->type;
			...
			$banners_url = $request->gallery_url;
			$countryData = array();		
					
			$countryUpdate = DB::table('constant_gallery')->where('gallery_id', $request->id)->update([
