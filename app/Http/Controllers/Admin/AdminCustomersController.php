...
{	
	...
	public function addcustomers(Request $request){
		...
		$discounted_products = DB::table('products_discount_price')->leftJoin('products_description', 'products_description.products_id', '=', 'products_discount_price.product_id')->get();

		$customerData['discounts'] = $discounted_products;
		$customerData['message'] = $message;
		$customerData['errorMessage'] = $errorMessage;
	...
	public function addnewcustomers(Request $request){
		...
		if(count($existEmail)>0){
			...
			$customers_id = DB::table('customers')->insertGetId([
						...
						'tier_lable'		 	 	    =>   $request->usertierlable,
						'customers_picture'	 			=>	 $customers_picture,
						'created_at'					=>	 time()
						]);
		if(!empty($request->discounted_product)){
		$discountData = array(
		    'customer_id'                   =>$customers_id,
		    'customer_tier'                 =>$request->usertierlable,
		    'discounted_product'            =>implode(', ', $request->discounted_product)
		);	
		}else{
		$discountData = array(
		    'customer_id'                   =>$customers_id,
		...
		$result_query = DB::table('customer_discounted_products')->where('customer_id', '=', $customers_id)->update($discountData);
			if($result_query==0){
			$product_ids     = $request->discounted_product!=''? implode(', ', $request->discounted_product) : '';
			DB::table('customer_discounted_products')->insert(['customer_id'=>$customers_id,'customer_tier'=>$request->usertierlable,'discounted_product'=>$product_ids]);
			//die('hhhhhhhhh');
			}		
	...
	public function editcustomers(Request $request){
		...
		$customers  = DB::table('customers')->where('customers_id','=', $customers_id)->get();
		$discounted_products = DB::table('products_discount_price')->leftJoin('products_description', 'products_description.products_id', '=', 'products_discount_price.product_id')->get();
        $assigned_product     = DB::table('customer_discounted_products')->where('customer_id','=', $customers_id)->get();


        $customerData['assigned_products'] = $assigned_product;
		$customerData['discounts'] = $discounted_products;
		$customerData['message'] = $message;
		$customerData['errorMessage'] = $errorMessage;
		$customerData['customers'] = $customers;
	...
	public function updatecustomers(Request $request){
		//print_r($request->discounted_product);
		//print_r(implode(',', $request->discounted_product));
		if(session('customers_update')==0){
			print Lang::get("labels.You do not have to access this route");
		}else{		
		$language_id            		=   '1';			
		...
		if($request->old_email_address!=$request->email){
			...
			if(count($existEmail)>0){
				...
				DB::table('customer_discounted_products')->where('customer_id', '=', $customers_id)->update($discountData);					 
				return redirect('admin/addaddress/'.$customers_id);		
			}
		}else{
			DB::table('customers')->where('customers_id', '=', $customers_id)->update($customer_data);
