...
//		//validation start
		if(empty($firstName)){
			$signup_error['firstName']='First Name Is Required';
		}
		if(empty($lastName)){
			$signup_error['lastName']='Last Name Is Required';
		}
		if(empty($email)){
			$signup_error['email']   ='Email  Is Required';
		}
		if(empty($phone)){
			$signup_error['phone']   ='Phone Is Required';
		}
		if(empty($company_name)){
			$signup_error['company_name']     ='Company Name Is Required';
		}
		if(empty($ABN)){
			$signup_error['ABN']     ='ABN Is Required';
		}
		if($password=='' || $password2==''){
			$signup_error['Password']     ='Password Is Required';
		}else{
			if ($password != $password2)
				 {
				    $signup_error['notmat']     ='Password Is Not Matched';
				 }
		}

		if(!empty($signup_error)){
		//print_r($signup_error);die('hiiiiiiiiiii');						
		$result['signup_error']  = $signup_error;
		return view("signup", $title)->with('result', $result);
		}else{
			...
			if(count($user_email)>0){
				...
				$signup_error['email_exist']  ='Email Already Exist';
				$result['signup_error']       = $signup_error;
		        return view("signup", $title)->with('result', $result);
